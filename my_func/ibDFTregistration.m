...
function [output, Greg, IMOut] = ibDFTregistration(buf1ft,buf2ft,IM,usfac)
%IBDFTREGISTRATION is modified function (M. Guizar - dftregistration) to 
%register additional data produced by original structural OCT data wich do 
%not have enough britghness to be registered apparent from associated 
%structural data.
% 
% For additional help see original function by Manuel Guizar
% DFTREGISTRATION
%
% Inputs:
%
%   IM     - appropriate frame for registration which do not have 
%            sufficient information for good registration
%
% Outputs
%
%   IMOut - registered frame which do not have sufficient information for
%           good registration
%
% See also: DFTREGISTRATION
...
if (nargout > 1)&&(usfac > 0),
    ...
    IMOut = IM.*exp(i*2*pi*(-row_shift*Nr/nr-col_shift*Nc/nc));
    IMOut = IMOut*exp(i*diffphase);
    
elseif (nargout > 1)&&(usfac == 0)
    Greg = buf2ft*exp(i*diffphase);
    
        IMOut = IM*exp(i*diffphase);

function circle = makeCircleTom(Nx, Ny, cx, cy, radius, arc_angle1, arc_angle2, plot_circle)
%MAKECIRCLE Create a binary map of a circle within a 2D grid.

% Tom edited this 8/21 so that you can specify a beginning and end angle
...
%     arc_angle1      - starting angle (0 starts at what I consider pi and goes counter clockwise) 
%     arc_angle2      - ending arc angle [radians]
%                       (default = 2*pi)
% OPTIONAL INPUTS:
...
for point_index = 1:length(px)
    ...
    % within the grid
    if ((atan2(px(point_index) - cx, py(point_index) - cy) + pi) <= arc_angle2) ...
            && ((atan2(px(point_index) - cx, py(point_index) - cy) + pi) >= arc_angle1)
